...
const EMPTY_FORM = (role: OfficerRole): CreateOfficerInput =>
  ({ name: '', login_id: '', role, password: '' });

export default function MyTeam() {
  const { user } = useAuth();
  const myRole = user!.role as OfficerRole;
  const qc = useQueryClient();

  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState<CreateOfficerInput>(EMPTY_FORM(myRole));
  const [formError, setFormError] = useState('');

  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['officers', myRole],
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setFormError('');
    createMut.mutate({ ...form, role: myRole });
  };
    ...
    <div className="p-8 max-w-5xl space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Officer accounts in your agency</p>
        </div>
        <button
      ...
      {isError   && <ErrorBanner message="Could not load team members." onRetry={refetch} />}

      {data && data.length === 0 && (
        <EmptyState title="No team members yet" description="Add the first officer in your agency using the button above." />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl bg-[#0a1628] border border-white/10 p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-white">Add Officer</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => { setShowModal(false); setForm(EMPTY_FORM(myRole)); setFormError(''); }}
                ...
                >
                  {createMut.isPending ? 'Adding…' : 'Add Officer'}
                </button>
              </div>
            </form>
          </div>
